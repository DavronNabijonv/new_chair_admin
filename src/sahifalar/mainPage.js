...
export default function MainPage() {
  ...
  const filtered_data = (e) => {
    ...
    if (id_response.length > 0) {  // Check if there are any matching items
    ...
    <div className="mainPage">
      <div className="grp">
        <form onSubmit={filtered_data} className="search">
          <input type="text" value={get_id} onChange={handleId} placeholder="Mebel ID raqami" />
          <button type="submit">Search</button>
        </form>
        <button onClick={()=>{setId_data(false)}} className="all_furniture">Barcha mebellar</button>
        <button
      ...
      <div className="data_body">
        {id_data?<OneData card_data={info_id}/>:<AllData item_infos={item} />}
      </div>
    </div>
  );
}
